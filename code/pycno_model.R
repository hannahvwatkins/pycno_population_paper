#this script prepares the data and runs the Stan model in R, then processes
#the output
...
abundance_summary_flat <- summary(abun_mod_rstan_flat)$summary
abundance_summary_flat %>% 
  as_tibble() %>% 
  mutate(rhat = round(Rhat,2)) %>% 
  filter(rhat != 1)


#store relevant parameters to export
export_params <- as.data.frame(abundance_summary) %>% 
  rownames_to_column() %>% 
  filter(!grepl("log_lik",rowname)) %>% 
  filter(!grepl("y_predict",rowname)) %>% 
  filter(!grepl("a_site",rowname)) %>% 
  filter(!grepl("a_dv",rowname))

export_params_flat <- as.data.frame(abundance_summary_flat) %>% 
  rownames_to_column() %>% 
  filter(!grepl("log_lik",rowname)) %>% 
  filter(!grepl("y_predict",rowname)) %>% 
  filter(!grepl("a_site",rowname)) %>% 
...
#examine sampling across sites and years---------------
reef_sites <- reef %>% 
  group_by(geogr,year) %>% 
  #number of return visits per year
  summarize(n=n()) %>% 
  ungroup() %>% 
  group_by(geogr) %>% 
  #number of years with return visits
  summarize(n=n()) %>% 
  rename(site=geogr) %>% 
  ungroup() %>% 
...
ow_sites <- ow %>% 
  ...
  #number of years with return visits
  summarize(n=n()) %>% 
  rename(site=place_name)%>% 
  ungroup()

iucn_sites <- iucn_pres_com %>% 
  group_by(location_code_numeric,year,source) %>% 
  #number of return visits per year
  summarize(n=n()) %>% 
  ungroup() %>% 
  group_by(location_code_numeric,source) %>% 
  #number of years with return visits
  summarize(n=n())%>% 
  rename(site=location_code_numeric)%>% 
  ungroup()
...
all_sites <- bind_rows(reef_sites, ow_sites, iucn_sites, aba_sites, multi_sites)
#total number of sites
nrow(all_sites)
#number with more than 1 year of sampling
nrow(filter(all_sites, n>1))
#proportion
nrow(filter(all_sites, n>1))/nrow(all_sites)
